...
class UnionConeType private constructor(
    ...
    companion object {
        ...
        fun ConeKotlinType.union(
            ...
            if (!skipValidCheck) {
                val valid = if (context != null) it.checkValid() else true
                if (!valid) {
                    it.isValid = false
                    if (nullIfNotValid) return@let null
                }
            }
        ...
        ) = UnionBuilder.of(declaration, substitutor, autoExpand, skipValidCheck, cached)
    }
    ...
    val resolvedUnion: List<ConeKotlinType>
        get() = _resolvedUnion.elseIfNull {
            if (context == null) {
                _resolvedUnion = _union ?: listOf()
                return@elseIfNull _resolvedUnion!!
            }
    ...
    val isNullable: Boolean
        get() = _isNullable.elseIfNull {
            _isNullable = context?.session?.let { thisType.canBeNull(it, true) } ?: false
            _isNullable!!
        }
    ...
    val rawAbbreviated: UnionConeType?
        get() = _rawAbbreviated.elseIfNull {
            if (context == null) {
                _rawAbbreviated = arrayOf(null)
                return@elseIfNull _rawAbbreviated!!
            }
    ...
    val withIntersection: UnionConeType?
        get() = _withIntersection.elseIfNull {
            if (context == null) {
                _withIntersection = arrayOf(null)
                return@elseIfNull _withIntersection!!
            }
    ...
    var cachedUnexpanded: UnionConeType? = null
        private set
    var diagnosticContext: Map<Int, UnionConeType>? = null
        private set
    ...
    fun substitute(other: ConeKotlinType, customSubstitutor: TypeSubstitutorMarker? = substitutor): ConeKotlinType? =
        context?.session?.typeContext?.let { customSubstitutor?.safeSubstitute(it, other) as? ConeKotlinType }
    context(context: CheckerContext?)
    ...
    fun toBuilder(autoExpand: Boolean = true, skipValidCheck: Boolean = true) = builder(declaration, substitutor, autoExpand, skipValidCheck, diagnosticContext)
    context(context: CheckerContext?, reporter: DiagnosticReporter?)
    ...
    fun checkValid(): Boolean {
        ...
        if (isIntersection && (
            ...
            report(
                ...
                a = intersected,
                b = this.withIntersectionOverride(listOf())
            )
            return false
        }
    ...
    context(context: CheckerContext)
    fun resolveAllForDiagnostics() {
        with(null as DiagnosticReporter?) {
            ...
            val unexpanded = cachedUnexpanded?.diagnosticContext ?: mapOf()
            if (resolved != this@UnionConeType) resolved?.resolveAllForDiagnostics()
            val resolved = resolved?.diagnosticContext ?: mapOf()

            val fromTypeArguments = mutableMapOf<Int, UnionConeType>()
            thisType.typeArguments.forEach { projection ->
                val type = projection.type?.let { copyTo(it) } ?: return@forEach
                type.resolveAllForDiagnostics()
                fromTypeArguments += type.diagnosticContext!!
            }
            expandedType.typeArguments.forEach { projection ->
                val type = projection.type?.let { copyTo(it) } ?: return@forEach
                type.resolveAllForDiagnostics()
                fromTypeArguments += type.diagnosticContext!!
            }

            val fromUnionAndIntersection = mutableMapOf<Int, UnionConeType>()
            unionWrapped.forEach { type ->
                type.resolveAllForDiagnostics()
                fromUnionAndIntersection += type.diagnosticContext!!
            }
            fullyResolvedUnionWrapped.forEach { type ->
                type.resolveAllForDiagnostics()
                fromUnionAndIntersection += type.diagnosticContext!!
            }

            diagnosticContext = hashMapOf(
                thisType.calculateHash() to this@UnionConeType,
                expandedType.calculateHash() to this@UnionConeType,
            ) + unexpanded + resolved + fromTypeArguments + fromUnionAndIntersection
        }
    }
}
